...
class URBaseEnv(gym.Env):
    ...
    def step(self, action, guarantee=False) -> Tuple[np.array, float, bool, dict]:
        ...
        if guarantee:
            rs_state = self.client.send_strict_action_get_state(rs_action.tolist()).state_dict
        else:
    ...
    otherwise returns list of position lists (standard index), since those are what get fed into step actions
        plan: [([joint_position1, joint_position2, ...], duration1), ...] list
            might add support for velocity, time_from_start etc. in the future
    '''
    def plan(self, goal):
        goal_list = [
                        goal.position.x,
        ...
        if plan.success:
            ...
            if max_diff <= 0.1:
                ...
                for pt in plan.trajectory:
                    # moveit plan already returns joint positions in standard index,
                    #   so no need to convert
                    # but need to convert absolute joint positions into proportional
                    #   as is the convention in robogym step
                    prop_jp_std_index = self.ur.normalize_joint_values(joints = pt.positions)
                    commands.append(prop_jp_std_index)
                    # min_traj_duration in ros_bridge is 0.5
                    # durations.append(max(pt.time_from_start - prev_time, 0.5))
                    # prev_time = pt.time_from_start
                return commands
                # return list(zip(commands, durations))
            else:
    ...
    def execute_plan(self, plan, skip_to_end=False):
        if not skip_to_end:
            for pt in plan[:-1]:
                # FIXME: wrist_3_joint is fixed in robogym but not in moveit
                self.step(pt[:-1], guarantee=True)

        return self.step(plan[-1][:-1], guarantee=True)


    '''
    helper function of execute_plan, preprocesses plan list in place
    '''
    def _prep_plan(self, plan):
        for pt in plan:
            action = pt[0]
