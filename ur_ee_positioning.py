...
from tf.transformations import random_quaternion, quaternion_from_euler
...
#     v = random.random()
#     w = random.random()
#     return np.array([sqrt(1-u)*sin(2*pi*v), sqrt(1-u)*cos(2*pi*v), sqrt(u)*sin(2*pi*w), sqrt(u)*cos(2*pi*w)])
...
class EndEffectorPositioningUR(URBaseEnv):
    ...
    def random_ee_pose_around_object0(self, translation_range=DISTANCE_THRESHOLD):
        result = Pose()

        rs_state = self.client.get_state_msg().state_dict
        self._check_rs_state_keys(rs_state)
        x = rs_state['object_0_to_ref_translation_x']
        y = rs_state['object_0_to_ref_translation_y']
        z = rs_state['object_0_to_ref_translation_z']
        result.position.x = random.uniform(x-translation_range, x+translation_range)
        result.position.y = random.uniform(y-translation_range, y+translation_range)
        result.position.z = random.uniform(z-translation_range, z+translation_range)

        rand_quat = random_quaternion()
        result.orientation.x = rand_quat[0]
        result.orientation.y = rand_quat[1]
        result.orientation.z = rand_quat[2]
        result.orientation.w = rand_quat[3]

        return result


    '''
    returns a random Pose object that is in action space
    '''
    def get_target_pose_quaternion(self):
        pose_euler = self._get_target_pose()
        quat = quaternion_from_euler(pose_euler[3], pose_euler[4], pose_euler[5])

        result = Pose()
        result.position.x = pose_euler[0]
        result.position.y = pose_euler[1]
        result.position.z = pose_euler[2]
    ...
    '''
    returns a moveit plan to object in list of joint-position lists
        each joint-position list cam be fed directly into step as action
        plan is [([joint_position1, joint_position2, ...], duration1), ...] list
    '''
    def get_moveit_plan_to_object0(self):
        for attempt in range(10):
            goal_pose = self.random_ee_pose_around_object0()
            moveit_plan = self.plan(goal_pose)
            if moveit_plan:
                return moveit_plan
        raise Exception('moveit plan to object0 failed')
    ...
    def reset(self, joint_positions = JOINT_POSITIONS, ee_target_pose = None, randomize_start=False) -> np.array:
        ...
        if ee_target_pose:
            ...
            high_enough = False
            while not high_enough:
                ee_target_pose = self._get_target_pose()
                # FIXME: remove this after planning scene and collision have been handled
                if ee_target_pose[2] >= 0.5:
                    high_enough = True
...
class URMaze(EndEffectorPositioningUR, Simulation):
    cmd = "roslaunch ur_robot_server ur_robot_server.launch \
